...
def create_workflow_for_job(
    session: Session, job: Any, payload: dict[str, Any], deadline_at: datetime
) -> ResearchWorkflow:
    """Create a workflow once and reuse its immutable input on every retry."""
    if job.workflow_id is not None:
        workflow = session.get(ResearchWorkflow, job.workflow_id)
        if workflow is None:
            raise ValueError("job references a missing workflow")
        if workflow.deadline_at is None:
            workflow.deadline_at = deadline_at
            workflow.started_at = datetime.now(UTC)
            workflow.status = "running"
        job.deadline_at = workflow.deadline_at
        return workflow
    ...
    job.stage_key = f"{job.kind}.research"
...
def persist_stage_result(
    ...
    *,
    ...
    workflow = session.scalar(
        ...
        .where(ResearchWorkflow.workflow_id == workflow_id)
        .with_for_update()
    )
    if workflow is None:
    ...
    latest = session.scalar(
        ...
        .where(
            ResearchStageResult.workflow_id == workflow_id,
